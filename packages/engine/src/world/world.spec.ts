...
import type { WorldErrorContext } from './world.types';
...
describe('World', () => {
  ...
  describe('prefab integration', () => {
    ...
    test('world.prefabs exposes the attached registry, or null', () => {
      const prefabs = new PrefabRegistry();
      const withRegistry = new World(Game.createHeadless(), {
        components: () => ({}),
        prefabs,
      });
    ...
    test('the same registry can be shared across multiple worlds', () => {
      ...
      const worldA = new World(Game.createHeadless(), {
        components: () => ({}),
        prefabs,
      });
      const worldB = new World(Game.createHeadless(), {
        components: () => ({}),
        prefabs,
      });
      ...
      const world: World = new World(Game.createHeadless(), {
        components: (w) => ({
          worldThing: () => ({
            ...
            onPostUpdate: () => events.push({ source: 'world', phase: 'post' }),
            onDestroy: () => {},
          }),
        }),
      });
